function uQF = build_compressed_B(ny, M, ny_window, use_Hann_window)
%BUILD_COMPRESSED_B build the compressed matrix u*Q*F for APF-c.
...
%
%   === Output Arguments ===
%   uQF (ny-by-M sparse matrix):
%       Matrix u*Q*F truncated to ny_window pixels around the peaks.
...
% Build the truncated version of u*Q*F as a sparse matrix.
m_L_window = 1:ny_window;
uQF = spalloc(ny, M, ny_window*M); % Pre-allocate the sparse matrix. 
for a = -a_min:1:a_min % Loop over input channels on the left.
    ...
    % Build u*Q*F within this window
    if use_Hann_window
        ...
        uQF(m, a+a_min+1) = term1 + term2 + term3;
    else
        % See Eq. (S36) in the supplementary of the APF paper.
        uQF(m, a+a_min+1) = (((-1)^a)/sqrt(ny*M))*sin((pi/ny*M)*m)./sin(pi*((m/ny)-(a/M)));
        % Handle situations where the denominator is zero.
        if int32(m_center)*M == int32(ny*(M+a))
            uQF(round(m_center)-ny*(a>0), a+a_min+1) = sqrt(M/ny);
        end
    end
end
