    ...
    % Example of a alternating sequence of layers of two dielectric material (also 
    % called a distributed Bragg reflector (DBR)) using SCSA 
    % 
    % Use MESTI2S() to compute 
    % 1.The wavelength-dependent reflectance spectrum from DBR and the conservation 
    % of energy (T + R = 1)
    ...
    % Calculate reflectance spectrum over visible wavelength for resolution = 60 
    ...
    % Over different resolution, compute L2 norm ratio error of numerical result 
    % with respect to the analytical to show discretization error.
